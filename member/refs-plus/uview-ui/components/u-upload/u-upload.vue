<template>
	<view class="u-upload" v-if="!disabled">
		...
		<u-popup v-model="showBtns" mode="bottom" width="750" border-radius="16" :mask-close-able="false">
			<view class="btns">
				<view class="" v-if="getBtn('camera')" @click="photograph">
					拍照
				</view>
				<view class="" v-if="getBtn('album')" @click="fromAlbum">
					从相册选择
				</view>
				<view class="cancel" @click="showBtns=false">
					取消
				</view>
			</view>
		</u-popup>
...
<script>
	...
	export default {
		...
		methods: {
			//获取权限
			getBtn(name) {
				return this.sourceType.some(item => {
					return item == name
				})
			},
			//获取时间
			...
			photograph() {
				...
				chooseFile = new Promise((resolve, reject) => {
					if (this.isImprint) {
						var watermark = uni.requireNativePlugin("WH-WatermarkPhoto")
						// #ifdef APP-PLUS
						const that = this
						watermark.useWatermarkPhotoWithOptions({
								'title': '客户跟踪',
								'weather': uni.getStorageSync('weather'),
								'location': uni.getStorageSync('address'),
								'time': that.dateFormat(that.date),
							},
							(ret) => {
								that.filePath = `file://${ret.imgPath}`
								let obj = {
									errMsg: "chooseImage:ok",
									tempFilePaths: [that.filePath],
									tempFiles: [{
										path: that.filePath,
										size: 2949276
									}]
