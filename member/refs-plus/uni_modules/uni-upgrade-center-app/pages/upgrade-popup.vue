	...
	<view class="mask flex-center">
		<view class="content botton-radius">
			...
			<view class="content-body">
				<view class="title" style='position: relative;z-index: 999;'>
					<text>红蚂蚁{{wgtVersion}}版本</text>
				...
				<view class="footer flex-center">
					<template v-if="isiOS">
						<button class="content-button"  plain @click="jumpToAppStore">
							{{downLoadBtnTextiOS}}
						</button>
					</template>
					<template v-else>
						<template v-if="!downloadSuccess">
							...
							<button v-else class="content-button" style="" plain @click="downloadPackage">
								{{downLoadBtnText}}
							</button>
						</template>
						<button v-else-if="downloadSuccess && !installed" class="content-button"
							 plain :loading="installing" :disabled="installing"
							@click="installPackage">
							{{installing ? '正在安装……' : '下载完成，立即安装'}}
						</button>

						<button v-if="installed && isWGT" class="content-button"  plain
							@click="restart">
							安装完毕，点击重启
						</button>
					</template>
	...
	export default {
		data() {
			return {
				...
				wgtVersion:uni.getStorageSync('wgtVersion')
			}
		},
		onLoad({
			local_storage_key
