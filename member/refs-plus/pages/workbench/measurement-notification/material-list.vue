<template>
	<view class="notified-detail">
		<u-navbar title="测量材料" :title-size='34' title-color='#333333' :titleBold="true" :border-bottom='false'
				  class="navbar">
		</u-navbar>
		<view class="center">
...
<script>
	export default {
		...
		onLoad(options){
			...
			console.log(info)
			info.materials?.forEach(el=>{
				el.select = false
			})
			this.list = this.$u.deepClone(info.materials)
			this.title = info.measureName
		},
		methods: {
			toNotice(){
				uni.navigateTo({
