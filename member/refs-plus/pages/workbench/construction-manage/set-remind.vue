		...
		methods: {
				...
				this.$post({
					...
					res.rows.forEach(item => {
						...
						if (this.customerIds.length!=0) {
							this.customerIds.forEach(item2 => {
								if (item2 == item.id) {
									item.selected = true
			...
			selectOne(item) {
				if (item.selected) {
					...
					this.names.splice(selectIndex, 1)
				} else {
					if (this.idName == 'chargePersonId') {
						this.indexList.forEach(val => {
							val.selected = false
						})
						this.customerIds[0] = item.id
						this.names[0] = item.name
					} else {
			...
			back() {
				...
				if (this.idName == 'chargePersonId') {
					prevPage.$vm.list[this.index][this.idName] = this.customerIds[0];
					prevPage.$vm.list[this.index][this.inputName] = this.names[0];
				} else {
					prevPage.$vm.list[this.index][this.idName] = this.customerIds;
					prevPage.$vm.list[this.index][this.inputName] = this.names;
				}
