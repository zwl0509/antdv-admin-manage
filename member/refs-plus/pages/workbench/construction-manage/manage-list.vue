					...
					<view class="list-y-r">
						<view class="list-y-r-info">
							<view class="list-y-r-info-y">
								<text class="name"><text style="font-weight: bold;">{{val.customerName}}</text> {{val.mobileNumber}}</text>
...
<!-- 							<view class="list-y-r-info-s">
						...
						<view class="list-y-r-time">
							<view class="list-y-r-time-l" @click="torecord(val)" >
								<text>{{val.inspectionRecordModifiedTime}} {{val.inspectionRecordName}}</text>
								<image src="/static/image/workbench/construction-manage/right.png" mode=""></image>
							</view>
							<view class="list-y-r-time-r">
								<image @click="toCheckType(val)" src="/static/image/workbench/construction-manage/icon-type.png"
									mode=""></image>
								<image @click="openConstructionPlan(val)"
									src="/static/image/workbench/construction-manage/icon-plan.png" mode=""></image>
							</view>
						</view>
					</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			torecord(val) {
				uni.navigateTo({
					url: '../customer-manage/record?customerId='+val.id
				})
			},
			// 施工计划
			openConstructionPlan(val) {
				uni.navigateTo({
					url: './construction-plan?customerId='+val.id+'&constructionPlanId='+val.constructionPlanId
				})
			},
			toCheckType(val) {
				uni.navigateTo({
					url: 'check-type?id='+val.id
				})
			}
		}
	}
