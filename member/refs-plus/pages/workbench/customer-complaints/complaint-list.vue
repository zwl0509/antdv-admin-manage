	...
	export default {
		...
		methods: {
			...
			todetail(val) {
				if (this.isCompleted) {
					uni.navigateTo({
						url: 'repair-detail?csComplaintRecordId=' + val.csComplaintRecordId +
							'&id=' + val.id
					})
				} else {
					uni.navigateTo({
						url: 'detail?csComplaintRecordId=' + val.csComplaintRecordId + '&id=' + val.id + '&role=' +
							val.employeeRole + '&customerId=' + val.customerId + '&type=' + this.type
					})
				}
