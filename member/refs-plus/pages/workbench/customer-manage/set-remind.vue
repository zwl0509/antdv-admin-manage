		...
		methods: {
			...
			assignEmployeeList() {
				this.$post({
					...
					this.customerIds=prevPage.$vm.reminder
					this.names=prevPage.$vm.reminderName
					res.rows.forEach(item => {
						item.selected = false
						prevPage.$vm.reminder.forEach(item2 => {
							if (item2 == item.id) {
								item.selected = true
							}
						})
			...
			back() {
				...
				let prevPage = pages[pages.length - 2]; //上一页页面实例
				uni.navigateBack({})
				prevPage.$vm.reminder = this.customerIds;
				prevPage.$vm.reminderName = this.names;
			}
		}
	}
</script>
