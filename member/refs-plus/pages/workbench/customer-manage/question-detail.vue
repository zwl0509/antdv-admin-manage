		...
		<view class="center">
			...
			<view class="userinfo">
				...
				<view class="userinfo-text">
					<view class="">
						{{customInfo.infoShow ? customInfo.customerName :customInfo.customerNameHide}}
					</view>
					<view class="">
						最近提交于{{createdTime}}
					</view>
			...
			<view class="question-y" v-for="(val,index) in qList" :key="index">
				...
				<view class="answer" v-if="val.questionType=='1077-10'">
					<input type="text" :value="val.surveyOptionsVOList[0].content" placeholder="请输入" :disabled="true" @input="aInput(index,$event)" />
				</view>
				<view class="answer" v-if="val.questionType=='1077-20'">
					<u-radio-group :wrap="true" v-model="val.value" @change="radioGroupChange" :disabled="true">
						<u-radio icon-size="16" @change="radioChange(item,index)" :disabledColor="false"
				...
				<view class="answer" v-if="val.questionType=='1077-30'">
					<u-checkbox-group :wrap="true" @change="checkboxGroupChange($event,index)" :disabled="true">
						<u-checkbox @change="checkboxChange" v-model="item.isCustomerPick" :disabledColor="false"
							v-for="(item, index2) in val.surveyOptionsInfoVOS" :key="index2" :name="item.id">
							{{item.content}}</u-checkbox>
					</u-checkbox-group>
				</view>
				<view class="answer" v-if="val.questionType=='1077-40'">
					...
					<input type="text" v-model="val.surveyOptionsVOList[0].content" placeholder="请选择" :disabled="true"/>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		onLoad(options) {
			this.createdTime=options.createdTime
				this.qList=JSON.parse(options.qList)
				this.qList.forEach(item=>{
					if(item.questionType=='1077-20'){
						item.surveyOptionsInfoVOS.forEach(item2=>{
							if(item2.isCustomerPick){
								item.value=item2.id
							}
						})
					}
			...
			checkboxGroupChange(e, index) {
				...
				this.qList[index].surveyOptionsInfoVOS.forEach(item => {
					e.forEach((item2, index) => {
						if (item.surveyQuestionId == item2) {
							a.surveyOptionsDTOList[index].score = item.score
						}
					})
				})
				a.surveyQuestionId = this.qList[index].id
