...
export default {
  ...
  methods: {
    ...
    loadData (pagination, filters, sorter) {
      ...
      const parameter = this.showPagination ? Object.assign({
    ...
    // 更新表头
    upDataColumns (list) {
      const new_columns = deepClone(list)
      const new_list = []
  ...
  render () {
    ...
    if (!this.$scopedSlots || !this.$scopedSlots.serial) {
      ...
      this.$scopedSlots.serialSingle = (txt, record, index) => (<span>{(index + 1)}</span>)
    }
    const table = (
