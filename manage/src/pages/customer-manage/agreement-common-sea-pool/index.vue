  ...
  <div>
    ...
    <list-page
      ...
      <template slot="$operate">
        <a-button v-if="actionAuth.includes('AgreementCommon.Create')" type="primary" icon="plus" @click="$refs.createModal.add(queryParam.type)">新建</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.Gain')" type="primary" icon="search" @click="getCustomer">捞取客户</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.BatchEvasionApply') && selectedRowKeys.length" type="primary" icon="edit" @click="$refs.EvasionApply.shows(selectedRowKeys,1)">批量逃单</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.BatchFutureHouse') && selectedRowKeys.length" type="primary" icon="edit" @click="$refs.FutureHouseApply.show(selectedRowKeys,'1056-20',2)">批量期房</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.BatchRetain') && selectedRowKeys.length" type="primary" icon="edit" @click="batchRetain()">批量保留</a-button>
        <a-popconfirm v-if="actionAuth.includes('AgreementCommon.BatchEliminate') && selectedRowKeys.length" title="是否要批量剔除选中数据？" @confirm="handleSub(selectedRowKeys)">
          <a-button type="primary" icon="delete">批量剔除</a-button>
        </a-popconfirm>
        <a-button v-if="actionAuth.includes('AgreementCommon.Download')" type="primary" class="download" icon="download" @click="downloadTemplate">下载模版</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.Import')" type="primary" class="download" icon="cloud-download" @click="uploadData">导入数据</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.Export')" type="primary" class="download" icon="download" @click="downloadData">导出数据</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.LogInfo')" type="primary" class="download" icon="eye" @click="$refs.ImportLogInfo.show('1066-40')">导入日志信息</a-button>
        <a-button v-if="actionAuth.includes('AgreementCommon.NewLog')" type="primary" class="download" icon="eye" @click="$refs.CustomerNewLog.show()">客户新增日志</a-button>
        <a-button type="primary" class="download" icon="cloud-download" v-if="actionAuth.includes('AgreementCommon.Import')" @click="uploadInfo">导入其他部门</a-button>
      </template>
      ...
      <span slot="action" slot-scope="text, record">
        <template v-if="!record.multiHouseStatus">
          <template v-if="actionAuth.includes('AgreementCommon.Modify') && !!!record.subManager">
            <a @click="$refs.createModal.edit(record,'edit',queryParam.type)">编辑</a>
            <a-divider type="vertical"/>
          </template>
          <template v-if="actionAuth.includes('AgreementCommon.Detail') && !!record.subManager">
            <a @click="$refs.createModal.edit(record,'detail',queryParam.type)">查看</a>
            <a-divider type="vertical"/>
          </template>
          <template v-if="!record.onCustomerType && actionAuth.includes('AgreementCommon.HandoverDesign')">
          ...
          <template v-if="actionAuth.includes('AgreementCommon.CustomerTracking')">
            ...
            <a-divider v-if="actionAuth.includes('AgreementCommon.EvasionApply')" type="vertical"/>
          </template>
          <template v-if="actionAuth.includes('AgreementCommon.EvasionApply')">
            <a @click="$refs.EvasionApply.show([record.id],2)">逃单申请</a>
          </template>
          <br>
          <template v-if="!record.future && !record.onFuture && actionAuth.includes('AgreementCommon.FutureHouseApply')">
            <a @click="$refs.FutureHouseApply.show([record.id],'1056-20',4)">期房申请</a>
            <a-divider type="vertical"/>
          </template>
          <template v-if="!record.onMultiHouse && actionAuth.includes('AgreementCommon.MuchHouserApply')">
            <a @click="$refs.createModal.apply(record,'apply',queryParam.type)">多房申请</a>
            <a-divider type="vertical"/>
          </template>
          <template v-if="!record.onRefluxed && actionAuth.includes('AgreementCommon.RetainApply')">
            <a @click="$refs.FutureHouseApply.show([record.bindId],'1056-10',3)">保留申请</a>
            <a-divider type="vertical"/>
          </template>
          <template v-if="actionAuth.includes('AgreementCommon.Eliminate')">
        ...
        <template v-else>
          <template v-if="actionAuth.includes('AgreementCommon.Modify')">
...
<script>
  ...
  import ReservationApply from '../second-common-sea-pool/modules/ReservationApply'
  import FutureHouseApply from '../primitive-common-sea-pool/modules/FutureHouseApply'
  import DispatchFrom from './modules/DispatchFrom'
  import DesignContract from './modules/DesignContract'
  ...
  export default {
    ...
    methods: {
      ...
      // 上传材料数据
      uploadData () {
        this.$refs.uploadModal.show(this.queryParam.type,'1066-40')
      },
      uploadInfo () {
        this.$refs.ImportDepartmentInfo.show(this.queryParam.type,'1066-40')
      },
      // 下载数据
      downloadData () {
