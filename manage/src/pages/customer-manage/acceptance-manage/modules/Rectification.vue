<template>
  <a-modal 
        ...
        <a-col :span="24" style="margin-bottom: 10px">
          ...
          <a-col :span="21">
            <a-select
              ...
              <a-select-option v-for="(s_item, s_index) in item.remindersList" :value="s_item.id" :key="s_index">
                {{ s_item.employeeName }}
              </a-select-option>
            </a-select>
          </a-col>
    ...
    <template slot="footer">
      ...
      <a-button type="primary" @click="handleSubmit">整改发送</a-button>
    </template>
  ...
  export default {
    ...
    methods: {
      show (list, id) { 
        ...
        data?.forEach(el => {
          if(el.detailVOS.length) {
            el.attachInfos = el.detailVOS[0].attachInfos
          }
        })
        this.dataList = data
        this.visible = true
      ...
      handleSubmit() {
        ...
        this.dataList.forEach((item,index)=> {
          ...
          const params = {
            ...
            customerConstructionTaskId: this.taskId,
