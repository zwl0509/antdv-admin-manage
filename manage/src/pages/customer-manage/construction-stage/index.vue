<template>
  <div>
    ...
    <list-page
      ref="listPage"
      class="small-table"
      columnsName="construction-stage"
      :showActionBtn="true"
      ...
      <template slot="$operate">
        <a-button v-if="actionAuth.includes('ConstructionStage.BatchEvasionApply') && selectedRowKeys.length" type="primary" icon="edit" @click="$refs.EvasionApply.shows(selectedRowKeys,1)">批量逃单</a-button>
        <a-button v-if="actionAuth.includes('ConstructionStage.Export')" type="primary" class="download" icon="download" @click="downloadData">导出数据</a-button>
      </template>
      ...
      <span slot="action" slot-scope="text, record">
        <template v-if="actionAuth.includes('ConstructionStage.Detail')">
          <a @click="$refs.createModal.edit(record,'detail',queryParam.type)">查看</a>
        </template>
        <template v-if="actionAuth.includes('ConstructionStage.CustomerTracking')">
          <a-divider type="vertical"/>
          <a @click="$refs.CustomerTracking.show(record, queryParam.type, actionAuth.includes('ConstructionStage.CreateCustomerTracking'))">客户跟踪</a>
        </template>
        <template v-if="actionAuth.includes('ConstructionStage.Evasion')">
          <a-divider type="vertical"/>
    ...
    methods: {
      ...
      getColumnX () {
        this.$get({
          ...
          params: { type: 'construction-stage'}
        }).then((res)=>{
      ...
      trackingProcess(record, item) {
        switch (item.workFlowRelationFormCode) {
          ...
          case '1059-30': // 设计表单
            ...
            break
          case '1059-40': // 施工表单
            this.$refs.SignContractFrom.show(record,item,'1033-80')
            break
          case '1059-50': // 附件表单
