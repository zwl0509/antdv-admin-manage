  ...
  <div>
    ...
    <list-page
      ...
      <template slot="$operate">
        <a-button v-if="actionAuth.includes('AgreementStage.Export') && selectedRowKeys.length" type="primary" icon="edit" @click="$refs.EvasionApply.shows(selectedRowKeys,1)">批量逃单</a-button>
        <a-button v-if="actionAuth.includes('AgreementStage.BatchEvasionApply')" type="primary" class="download" icon="download" @click="downloadData">导出数据</a-button>
      </template>
      ...
      <span slot="action" slot-scope="text, record">
        <template v-if="actionAuth.includes('AgreementStage.Detail')">
          <a @click="$refs.createModal.edit(record,'detail',queryParam.type)">查看</a>
          <a-divider type="vertical"/>
        </template>
        <template v-if="actionAuth.includes('AgreementStage.CustomerTracking')">
          <a @click="$refs.CustomerTracking.show(record, queryParam.type, actionAuth.includes('AgreementStage.CreateCustomerTracking'))">客户跟踪</a>
          <a-divider type="vertical"/>
        </template>
        <template v-if="actionAuth.includes('AgreementStage.EvasionApply')">
...
<script>
  ...
  import ActionModal from '@/pages/customer-manage/agreement-common-sea-pool/modules/ActionModal'
