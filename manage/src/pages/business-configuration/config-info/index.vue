  ...
  <div>
    ...
    <list-page
      ...
      <template slot="$operate">
        <a-button v-if="actionAuth.includes('ConfigInfo.Create')" type="primary" icon="plus" @click="$refs.createModal.add()">新建</a-button>
      </template>
      <span slot="action" slot-scope="text, record">
        <template v-if="actionAuth.includes('ConfigInfo.Modify')">
          <a @click="handleEdit(record)">编辑</a>
          <a-divider type="vertical"/>
        </template>
        <template v-if="actionAuth.includes('ConfigInfo.Delete')">
...
export default {
  ...
  data () {
    return {
      loading: false,
      requestUrl: this.$api.allocation.customerAscriptionInfo.getListPage,
      // 查询参数
      queryParam: {},
      searchParams: () => {
