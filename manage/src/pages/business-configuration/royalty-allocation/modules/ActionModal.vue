    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-row :grabbed="48">
          ...
          <a-col :md="12" :xs="24">
            <a-form-item label="提成百分比" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input type="number" placeholder="请输入提成百分比" autocomplete="off" v-decorator="['percentage']"/>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :grabbed="48">
...
export default {
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.$get({
        url: this.$api.allocation.CommissionInfo.getDetail,
        params: { id: record.id }
      })
        .then(res => {
          const data = { ...res }
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          values.id = this.id
          this.$post({
            url: this.$api.allocation.CommissionInfo.edit,
            data: values,
            needResponse: true
          })
            .then(res => {
