  ...
  <div>
    <a-form :form="form">
      <a-row :grabbed="48">
        <a-col :md="8" :xs="24">
          <a-form-item label="客户名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-tooltip v-if="customerName" :title="customerName" :disabled="type == 'detail'">
              <div v-if="customerName" class="customer-name">{{ customerName || '' }}</div>
            </a-tooltip>
            <a-button type="primary" :disabled="type == 'detail'" @click="selectOrg">选择客户名称</a-button>
          </a-form-item>
        </a-col>
        <a-col :md="8" :xs="24">
          <a-form-item label="地址" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-tooltip v-if="finalAddress" :title="finalAddress">
              <div v-if="finalAddress" class="dispatch-user">{{ finalAddress || '请输入地址' }}</div>
            </a-tooltip>
            <a-input
              v-else
              placeholder="请输入地址"
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="设计部经理" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入设计部经理"
              autocomplete="off"
              disabled
              v-decorator="['designManager']"/>
          </a-form-item>
        </a-col>
        <a-col :md="8" :xs="24">
          <a-form-item label="软装设计师" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入软装设计师"
              autocomplete="off"
              disabled
              v-decorator="['signDate1']"/>
          </a-form-item>
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="宅配设计师" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入宅配设计师"
              autocomplete="off"
              disabled
              v-decorator="['signDate1']"/>
          </a-form-item>
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="品管" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入品管"
              autocomplete="off"
              disabled
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="市场部" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入市场部"
              autocomplete="off"
              disabled
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="决算主材部分" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-input
              placeholder="请输入决算主材部分"
              autocomplete="off"
              disabled
              v-decorator="['signDate1']"/>
          </a-form-item>
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="保修截止日期" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-date-picker
              show-time
        ...
        <a-col :md="8" :xs="24">
          <a-form-item label="投诉时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-date-picker
              show-time
              placeholder="请选择投诉时间"
              style="width: 100%;"
              :disabled="type == 'detail'"
              v-decorator="['recordTime',{ rules : [{ required: true, message: '请选择投诉时间!'}]}]"
            ></a-date-picker>
          </a-form-item>
        </a-col>
        <a-col :md="8" :xs="24">
          <a-form-item label="投诉来源" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-select allowClear placeholder="请选择投诉来源" :disabled="type == 'detail'" v-decorator="['recordSource', {rules: [{required: true}]}]">
              <a-select-option
                v-for="(item, index) in codeType.complaintSource"
                :key="index"
                :value="item.value"
      ...
      <a-row>
        <a-col :md="24" :xs="24">
          <a-form-item label="投诉类别" :labelCol="labelCol" :wrapperCol="wrapperCol" class="col-1-9" >
            <a-select
              showArrow
              allowClear
              show-search
              ...
              placeholder="请选择投诉类别"
              :disabled="type == 'detail'"
              v-decorator="['recordTypes',{ rules : [{ required: true, message: '请选择投诉来源!'}]}]">
              <a-select-option
                v-for="(item, index) in codeType.complaintType"
                :key="index"
                :value="item.value"
              >{{ item.name }}</a-select-option>
            </a-select>
...
<script>
  ...
  import ActionModal from '@/pages/customer-service-manage/complaint-record/modules/ActionModal'

  export default {
    name: 'ComplaintInfo',
    components: { CustomerInfo,Ellipsis },
    props: {
      type: {
        type: String,
        default: '',
